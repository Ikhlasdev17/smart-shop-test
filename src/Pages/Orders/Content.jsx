...
const Content = ({
  ...
  useEffect(() => {
    setSpinning(true);
    axios
      .get(`${URL}/api/orders?basket_id=${currentBasketItemId}`, setToken())
      .then((res) => {
  ...
  const handleCheckedAll = (e) => {
    ...
    if (e.target.checked) {
      basketItems.map((item) => {
        ...
        setNewArr((prev) => [
          ...prev,
          { order_id: item.id, count: item.count },
        ]);
      });
    } else {
      setNewArr([]);
  ...
  basketItems?.map((item) => {
    dataSource.push({
      ...
      select: (
        <Checkbox
          checked={checkedItems.findIndex((x) => x === item?.id) !== -1}
          onChange={(e) => singleItemChecked(e, item)}
        />
      ),
      ...
      minus: (
        <>
          <InputNumber
            value={newArr.filter((x) => x.order_id === item?.id).count}
            onChange={(e) => handleChangeBasketItem(e, item)}
            disabled={selectedKeys?.indexOf(item?.id) === -1}
          />
        </>
      ),
    });
    ...
    <div>
      ...
      <div>
        <iframe
          width="280"
          height="280"
          className="qr_code_frame"
          src={basket.qr_link}
          title="Bu yerda mahsulotning qr kodi bolishi kerak edi!"
        ></iframe>
      </div>
    </div>
  );
